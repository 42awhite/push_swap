...
/*   Updated: 2023/03/06 20:40:10 by ablanco-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_line(char *buffer)
{
	...
	while ((buffer[pos_n] != '\n' && buffer[pos_n] != '\0'))
			pos_n++;
	if (buffer[pos_n] == '\n')
		str_line = ft_gnl_calloc(pos_n + 2, sizeof(char));
	else
		str_line = ft_gnl_calloc(pos_n + 1, sizeof(char));
	if (!str_line)
		return (NULL);
	pos_n = -1;
	while (buffer[++pos_n] != '\n' && buffer[pos_n] != '\0')
...
char	*save_txt(char *buffer)
{
	...
	txt = ft_gnl_calloc(ft_gnl_strlen(buffer) - pos_n + 1, sizeof(char));
	c_txt = 0;
	pos_n++;
	while (buffer[pos_n])
	{
